...
class Game(object):
    ...
    def _get_tracking_data(self):
        ...
        """
        os.system("7za -o../data/temp x ../data/movement_data/" + self.tracking_id + ".7z")
        ...
        with open('../data/temp/{self.game_id}.json'.format(self=self)) as data_file:
            self.tracking_data = json.load(data_file)  # Load this json
        os.remove('../data/temp/{self.game_id}.json'.format(self=self))
    ...
    def _get_playbyplay_data(self):
        ...
        self.pbp = pd.read_csv('../data/movement_data/events/{self.game_id}.csv'.format(self=self))
        #Additional formating for the game id
        self.pbp['GAME_ID'] = '00' + self.pbp['GAME_ID'].astype(int).astype(str)
    ...
    def _get_player_ids(self):
        ...
        self.home_team_info['players_id'] = self.players_name_lookup(self.home_team_info)
        self.visitor_team_info['players_id'] = self.players_name_lookup(self.visitor_team_info)
        print(self.home_team_info['players_id'])
        print(self.visitor_team_info['players_id'])
        return self

    def players_name_lookup(self, players_list):
        lookup = dict()
    ...
    def _format_tracking_data(self):
        ...
        self.home_team_info = events["home"]
        self.visitor_team_info = events["visitor"]
        moments = []
        ...
        #time played by adding 720 seconds for each quarter played and the current quarter time
    ...
    def _determine_direction(self):
        """
        Helper function to determine which direction the home team is going.
